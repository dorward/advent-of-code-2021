...
const isThisAWinningBoard = (board: Board): boolean => {
  if (board.find((row) => row.every((cell) => cell.checked))) return true;
...
const getLosingBoards = (boards: Board[]): Board[] =>
  boards.filter((board: Board) => !isThisAWinningBoard(board));
...
(async () => {
  ...
  let boards = createBoards(numbers, lines);
  ...
  for (let i = 0; i < numbers.length; i++) {
    ...
    printBoards(boards);
    boards = getLosingBoards(boards);
    if (boards.length === 0) {
      process.stdout.write(chalk.green(`${number} is the last to win!\n`));
      showWin(previousBoards[0], number);
      break;
    }
